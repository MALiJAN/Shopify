#!/usr/bin/env python3
# shopify_api_standalone.py
# Shopify Card Checker API Server
# Usage: python shopify_api_standalone.py
# Or with gunicorn: gunicorn -w 4 -b 0.0.0.0:5000 shopify_api_standalone:app
...
def check_and_install(package):
    ...
    try:
        importlib.metadata.version(package.replace("-", "_").lower())
...
# ── Selenium CAPTCHA solver (disabled) ──────────
...
_CAPTCHA_MAX_RETRIES = 0  # Disabled CAPTCHA retries
...
async def run_shopify_check(site_url, card_str, proxy_url=None, verbose=False, discord_console_webhook=None, timeout=120.0, max_captcha_retries=None):
    ...
    async with _check_semaphore:
        _active_checks += 1
        try:
            return await _run_shopify_check_inner(site_url, card_str, proxy_url, verbose, discord_console_webhook, timeout, max_captcha_retries)
        finally:
            _active_checks -= 1
...
async def _run_shopify_check_inner(site_url, card_str, proxy_url=None, verbose=False, discord_console_webhook=None, timeout=120.0, max_captcha_retries=None):
    ...
    if _is_captcha_result(result):
        ...
        result["message"] = "CAPTCHA detected, site skipped"
        return result
...
def shopify():
    """
    Check a card on a Shopify site.
    Parameters:
        site (str): The Shopify site URL (e.g., https://example.myshopify.com)
        cc (str): Card in format CC|MM|YY|CVV
        proxy (str, optional): Proxy URL (e.g., http://user:pass@ip:port)
    Returns:
        JSON with status, message, price, etc.
    """
...
def health():
    ...
    return jsonify({"status": "ok", "workers": "unlimited"})
...
def index():
    ...
    return jsonify({
        ...
        "version": "1.0",
        "description": "Check credit cards on Shopify stores",
        "requirements": "Proxy is required for every request",
        "endpoints": {
            "/shopify": "GET with params: site, cc, proxy (required)",
...
if __name__ == '__main__':
    print("=" * 50)
    print("🚀 Starting Shopify API Server")
    print("📍 Running on http://0.0.0.0:5000")
    print("🔧 Threaded mode: ON (unlimited concurrent requests)")
    print("📡 Endpoint: /shopify?site=<url>&cc=<card>&proxy=<proxy>")
    print("💡 Proxy is REQUIRED for every request")
    print("=" * 50)
    
    # Run with Flask's built-in server (threaded=True for concurrency)
